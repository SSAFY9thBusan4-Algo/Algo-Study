...
네구간으로 나누어 재귀를 이용하여 풀었다. 
  

### 👀어려웠던 점

예제는 문제 없이 잘 수행되었으나 계속 오답처리가 되었다. 이후 질문게시판을 찾아봤고, 반례들을 넣어봤다. 

2 11 11 입력시 1이 나와야하는데 (1111)이 나왔다. 그래서 처음부터 값들이 모두 같을 때는 따로 처리를 해주니 이상없이 돌아갔다.